...
import baseDataGenerateProcess_zxptsystem.coresystem.InstInfoDataGenerate;
import baseDataGenerateProcess_zxptsystem.report.ReportInstInfoDataGenerate;
import baseDataGenerateProcess_zxptsystem.report.TaskFlowDataGenerate;
import baseDataGenerateProcess_zxptsystem.zxptsystem.JGZXReportGenerate;
import baseDataGenerateProcess_zxptsystem.zxptsystem.ZXBaseDataGenerate;
import baseDataGenerateProcess_zxptsystem.zxptsystem.ZXReportReviewed;
...
@SuiteClasses(value={
	...
	JGZXReportGenerate.class
})
public class JGZXGenerateReportTest extends ActionTestCase {
	...
	public static void setSuitInfo(){
		...
		try {
			...
			condition.add("0~2~1~24");
			condition.add("0~0~1~mavenTest12");
			ExcelUtils.updateExcelValue("ReportInstInfo.xls", condition, "~");
			// 修改任务的主题
			dtoName = "report.dto.TaskFlow";
			ExcelUtils.updateExcelValue("TaskFlow.xls", "0~8~1~24", "~");
	...
	public static void deleteTestData(){
		try {
			...
			// 删除企业征信报文文件保存路径
			ActionTestUtils.deleteFilePath(System.getProperty("java.io.tmpdir")+"JGZXReportFile");
		} catch (Exception e) {
			e.printStackTrace();
