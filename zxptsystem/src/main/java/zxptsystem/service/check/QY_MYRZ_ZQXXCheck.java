...
/***
 * 贸易融资业务-融资业务-展期信息段
 * 同一基础段且业务发生日期相同的情况下，同一融资业务展期信息只可存在一笔
 * @author xiajieli
 *
 */
public class QY_MYRZ_ZQXXCheck implements ICheckWithParam {
	...
	public MessageResult Check(Map<String, Object> mapObject) throws Exception {
		...
		detachedCriteria = DetachedCriteria.forClass(Class.forName("zxptsystem.dto.AutoDTO_QY_MYRZ_ZQXX"));
		...
		if(objectList.size()>=1){
			...
			messageResult.getMessageSet().add("同一基础段且业务发生日期相同的情况下，同一融资业务展期信息只可存在一笔");
		}
