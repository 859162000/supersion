 ...
 *
 */
public class ConfirmDialogForOverRideQYZXReportService implements IObjectResultExecute {
	...
	public Object objectResultExecute() throws Exception {
		...
		try{
			...
			if(RequestManager.getActionName().contains("GetJsonForDownLoadReport-zxptsystem.dto.QYZXDownload")){
			...
			else if(RequestManager.getActionName().contains("GetJsonForOverrideDownLoadReport-zxptsystem.dto.OverRideGRZXDownload")){
				...
				//liu
				String endGRSJFSNY = (String) ReflectOperation.getFieldValue(RequestManager.getTOject(), "endGRSJFSNY");
				String beginGRSJFSNY = (String)ReflectOperation.getFieldValue(RequestManager.getTOject(), "beginGRSJFSNY");
				SimpleDateFormat format=new SimpleDateFormat("yyyyMMdd");
				Date endDate=null;
				Date beginDate=null;
				try {
					 endDate=format.parse(endGRSJFSNY);
					 beginDate=format.parse(beginGRSJFSNY);
				} catch (Exception e) {}
				if(endDate.getTime()<beginDate.getTime()){
					messageResult.getMessageSet().add("alert结束时间不能早于开始时间");//注：alert不能删
					messageResult.setSuccess(false);
					messageResult.AlertTranslate();
					ServletActionContext.getContext().getApplication().put(key, null);
					return messageResult;
				}
				ActionContext.getContext().getSession().put("endGRSJFSNY", endGRSJFSNY);
				ActionContext.getContext().getSession().put("beginGRSJFSNY", beginGRSJFSNY);
					...
					if ("1".equals(strGrReportType)) {
						...
						for (int i = 0; i < selectReport.length; i++) {
							...
							while(beginDate.getTime()!=endDate.getTime()){
								strList.add(endGRSJFSNY);
								endGRSJFSNY=this.getBeforeDay("yyyyMMdd", endGRSJFSNY);
								endDate=new SimpleDateFormat("yyyyMMdd").parse(endGRSJFSNY);
								 }
							if(beginDate.getTime()==endDate.getTime()){
								strList.add(endGRSJFSNY);
							}
							
							detachedCriteria.add(Restrictions.in("JSYHKRQ",strList));
							 objectList=(Integer)singleObjectFindByCountDao.paramObjectResultExecute(new Object[]{detachedCriteria,null});
							
							if(objectList==0){
								ServletActionContext.getContext().getApplication().put(key, null);
								messageResult.getMessageSet().add("alert本段时间内没有数据，终止生成报文!");//注：alert不能删
								messageResult.setSuccess(false);
								messageResult.AlertTranslate();
								ServletActionContext.getContext().getApplication().put(key, null);
								return messageResult;
						...
						if(objectList == 0){
							ServletActionContext.getContext().getApplication().put(key, null);
							  return new MessageResult(false,strNoData);
						}
						if(objectTempList> objectList){
							ServletActionContext.getContext().getApplication().put(key, null);
							alertMessages += strExistUntreatedData;
  ...
  private String getBeforeDay(String from,String dNow){
