 ...
 *
 */
public class DownLoadGRZXRportService implements IObjectResultExecute {
	...
	public Object objectResultExecute() throws Exception {
		...
		try{
			...
			if(!StringUtils.isBlank(strGrReportType) && strGrReportType.equals("1")){
				if(StringUtils.isBlank(strSJFSNY)){
					messageResult.getMessageSet().add("数据发生年月不能为空");
				}
			}
	...
	private Object GenerateReport() throws Exception {
		...
		if ("1".equals(strGrReportType)) {//正常报文
			...
			if(!StringUtils.isBlank(this.strSJFSNY)){
				fileName += this.strSJFSNY;
			}else{
				fileName += HelpTool.getBeforeDate("yyyyMM");
			}
			...
			for (int i = 0; i < selectReport.length; i++) {
				...
				detachedCriteria.add(Restrictions.like("JSYHKRQ", this.strSJFSNY+"%"));
		...
		new DownloadResourceService().DownloadResource(filePath.replace(".txt", ".zip"), "22", "1",fileDescription); // 下载资源库新增一条数据
