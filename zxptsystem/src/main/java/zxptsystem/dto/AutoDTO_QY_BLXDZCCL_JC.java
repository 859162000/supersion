...
import zxptsystem.dto.AutoDTO_QY_BLXDZCCL_BS;
import zxptsystem.dto.AutoDTO_QY_CZXX;

@Entity
@Table(name = "QY_BLXDZCCL_JC")
@IEntity(description= "不良信贷资产处置信息基础段")
public class AutoDTO_QY_BLXDZCCL_JC implements java.io.Serializable{
	...
	@Column(name = "GSZCDJH", length = 50, nullable = true)
	@IColumn(description="工商注册登记号")
	private String GSZCDJH;

	public String getGSZCDJH() {
		return GSZCDJH;
	}

	public void setGSZCDJH(String in) {
		GSZCDJH = in;
	}

	@Column(name = "YWBH", length = 200, nullable = true)
	@IColumn(description="业务编号")
	private String YWBH;
	...
	public static Map<String,String> getXXJLCZLXTag() {
		try {
			return HelpTool.getSystemConstVal("XXJLCZLX1");
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	private Set<zxptsystem.dto.AutoDTO_QY_BLXDZCCL_BS> OneToMany_QY_BLXDZCCL_BS= new HashSet<zxptsystem.dto.AutoDTO_QY_BLXDZCCL_BS>(0);

	public Set<zxptsystem.dto.AutoDTO_QY_BLXDZCCL_BS> getOneToMany_QY_BLXDZCCL_BS() {
		return OneToMany_QY_BLXDZCCL_BS;
	}

	public void setOneToMany_QY_BLXDZCCL_BS(Set<zxptsystem.dto.AutoDTO_QY_BLXDZCCL_BS> in) {
		OneToMany_QY_BLXDZCCL_BS = in;
	}

	@OneToMany(cascade = CascadeType.ALL, fetch = FetchType.LAZY, mappedBy = "FOREIGNID")
	private Set<zxptsystem.dto.AutoDTO_QY_CZXX> OneToMany_QY_CZXX= new HashSet<zxptsystem.dto.AutoDTO_QY_CZXX>(0);

	public Set<zxptsystem.dto.AutoDTO_QY_CZXX> getOneToMany_QY_CZXX() {
		return OneToMany_QY_CZXX;
	}

	public void setOneToMany_QY_CZXX(Set<zxptsystem.dto.AutoDTO_QY_CZXX> in) {
		OneToMany_QY_CZXX = in;
	}
