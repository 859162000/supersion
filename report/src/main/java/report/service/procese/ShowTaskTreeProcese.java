 ...
 * @version 1.0<br>    
 */
public class ShowTaskTreeProcese implements IProcese {
	...
	public Object Procese(Object serviceResult) throws Exception {
		...
		for (Object object : objectList) {
			...
			if (!taskFlow.getStrTaskState().equals("1") || (taskFlow.getTaskStartData().getTime() > new Date().getTime()) || ((taskFlow.getTaskEndData().getTime() + 86400000) < new Date().getTime())) {
				...
				node.setURL("report/ShowTaskRptInstToMeger-report.dto.TaskRptInst.action?id="+taskFlow.getId()+"&windowId="+RequestManager.getWindowId());
