...
<html>
	<head>
		...
		<script type="text/javascript">
	        ...
	        function ReportMakeConfirm(actionNamespace,actionName,page){
	            var reportType=$("#serviceParam.strReportType").val();
	            var ywfsrq=$("#from1_serviceParam_YWFSRQ").val();
	            var jrjgcode=$("#serviceParam.strJRJGCode").val();

	            var params=$("#from1").serialize();

	            var u1="<%=basePath %>ajaxForReport/GetJsonForDownLoadReport-zxptsystem.dto.QYZXDownload.action";
				...
				$.ajax({
		            ...
		            else
			        {
		            	if(result.message.indexOf("alert")>0)
					     {
		            		conRes = alert(result.message.substring(9));
					     }
		            	 else if(result.message.indexOf("check")>0)
		            	 {
		            		conRes = alert(result.message.substring(9));
				         }
					     else
					     {
					    	 conRes = confirm(result.message);
						 }
			        }
			            
					if(conRes==true)
					{
						var actionname="DownLoadRport-zxptsystem.dto.QYZXDownload.action";
    ...
    function onSelectChange(){
		document.form1.action="<%=basePath%>framework/ShowList-zxptsystem.dto.QYZXDownload.action";
		document.form1.submit();
	}
	...
	<body onResize="AutiSize();" style="margin:0px; width: 100%;height:100%;" onload="ShowHeadName();" ondragstart="return false;">
		...
		<s:form name="form1" enctype="multipart/form-data" method="POST" id="from1">
			<div id="SelectDivFrame" style="width: 100%; height:auto;border-bottom:1px solid #0060A6;font-size: 12px; font-family: 微软雅黑;clear: both;background-color:White;z-index:1000">
						...
						<div class="Select_Menu_Control_Frame_Div">
							...
							<div class="Select_Menu_SelectBox_Div">
								<div class="Select_Menu_SelectBox_Div_Hidden">
								    <s:if test="%{#sr.paramName=='strReportType'}"><s:select cssClass="Select_Menu_SelectBox_Div_Option"
										list="#sr.tag"
								    ...
								    <s:else>
										...
										headerKey="%{#application.emptySelectValueName}"
										headerValue="---请选择---" 
										onmousemove="changeOptionsTitle('serviceParam.%{#sr.paramName}');"/>							    </s:else>
								</div>	
							</div>
						</div>
					</s:elseif>
				...
				<s:iterator value="serviceResult.operationList" id="sr">
					<s:if test="%{#sr.operationType=='Select'}">
						<div  class="Select_Menu_Control_Frame_Div" style="text-align:center;">
						<div style="margin-left:auto; margin-right:auto; width:130px; height:30px;margin-top:5px;">
						<a style="float: left; line-height:25px; color:White; text-decoration:none;font-family:微软雅黑;font-size:12px; cursor:pointer; padding-left:15px; padding-right:15px; background-color:#0060A6; margin-left:1px;width:100px;"
				...
				<s:iterator value="serviceResult.operationList" id="sr">
					<s:if test="%{#sr.operationType=='Select1'}">
						<div  class="Select_Menu_Control_Frame_Div" style="text-align:center;">
						<div style="margin-left:auto; margin-right:auto; width:130px; height:30px;margin-top:5px;">
						<a style="float: left; line-height:25px; color:White; text-decoration:none;font-family:微软雅黑;font-size:12px; cursor:pointer; padding-left:15px; padding-right:15px; background-color:#0060A6; margin-left:1px;width:100px;"
							href="javascript:ReportMakeConfirm('<s:property value="operationNamespace"/>','<s:property value="operationAction"/>')"><s:property value="operationName" /></a>
						...
						<s:iterator value="serviceResult.valueTable" id="vt" status="st">
							<tr class="JS_Grid_Data_Row" style="font-size: 12px; font-family: 微软雅黑;" Rows_Type='DataRow'>
								...
								<s:iterator value="serviceResult.linkedList.get(#st.index)" id="ll">
									...
									<s:elseif test="%{#ll.operationType=='Select1'}"> 
										<div title="<s:property value='operationName'/>" class="JS_Grid_Data_Row_CellLink" style="width: <s:property value="(width==null || width=='')?100:width" />px;" columncode="ColA">
											<a href="javascript:ReportMakeConfirm('<s:property value='%{#ll.operationNamespace}'/>','<s:property value='%{#ll.operationAction}'/>','<s:property value='%{#vt.get(0).value}'/>','<s:property value='%{#vt.get(1).value}'/>','<s:property value='%{#request.windowId}'/>')"  style="display:inline-block;color:%{#ll.color};">
												<s:if test="%{#ll.imageUrl==null||imageUrl.equals('')}">
													<s:property value="operationName" />
												</s:if>
												<s:else>
