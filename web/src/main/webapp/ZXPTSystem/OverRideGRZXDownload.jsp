...
<html>
	<head>
		...
		<script type="text/javascript">
	        ...
	        function ReportMakeConfirm(actionNamespace,actionName,page){
	            var reportType=$("#serviceParam.strGrReportType").val();
	            var jrjgcode=$("#serviceParam.strGrJRJGCode").val();
	            var ywfsrq=$("#serviceParam.strGRSJFSNY").val();
	            var params=$("#from1").serialize();
	            var u1="<%=basePath %>ajaxForReport/GetJsonForOverrideDownLoadReport-zxptsystem.dto.OverRideGRZXDownload.action";
				$.ajax({
					type:"post",
		            url:u1,
		            data:params,
					...
					if(conRes==true)
					{
						var actionname="OverRideDownLoadGRZXRport-zxptsystem.dto.OverRideGRZXDownload.action";
    ...
    function onSelectChange(){
		document.form1.action="<%=basePath%>framework/ShowApply-zxptsystem.dto.OverRideGRZXDownload.action";
		document.form1.submit();
	}
			...
			<div id="SelectDivFrame" style="width: 100%; height:auto;border-bottom:1px solid #0060A6;font-size: 12px; font-family: 微软雅黑;clear: both; top:30px; background-color:White;z-index:1000">
				<s:iterator value="serviceResult.showCondition" id="sr" status="i">
					...
					<s:elseif test="%{#sr.singleTag=='selectField'}">
						<div class="Select_Menu_Control_Frame_Div">
							<div class="Title"  title="<s:property value="%{#sr.fieldShowName}"/>" class="Font_css_normal_Condtion">
								<s:property value="%{#sr.fieldShowName}" /><br />
							</div>
							<div class="Select_Menu_SelectBox_Div">
								<div class="Select_Menu_SelectBox_Div_Hidden">
									<s:if test="%{#sr.paramName=='strGrReportType'}"><s:select cssClass="Select_Menu_SelectBox_Div_Option"
										list="#sr.tag"
