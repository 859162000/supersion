...
<%--					WhenUserClikeDiv("SelectDivFrame");		lanyuesheng	20160203	据条件是否存在值显示/隐藏 --%>
    ...
    <script type="text/javascript">
		...
		function ReportImportByAjax(actionNamespace,actionName){
			var instCode = $("#instList").val();
			if(instCode == null || instCode == "") {
				alert("请选择机构信息");
				return false;
			}
            var index = $("#123456").val().lastIndexOf("\\")+1;
			var fileName = $("#123456").val().substring(index);
			...
			$.ajax({
	            ...
	            success:function(data){
					if(data.success) {
						var path = '<%=basePath %>framework/ReportImport-bwdrsystem.dto.ReportFileImportLog.action';
						$("#from1").attr("action", path)
						$("#from1").submit();
					} else {
	...
	<script type="text/javascript">
		$(document).ready(function(){
			var url="<%=basePath %>framework/GetInstInfoJson.action";
			$.ajax({
				type:"get",
	            url:url,
	            datatype: "json",
	            success:function(data){
	            	var select = $("#instList");
	            	$("<option value=\"\" selected=\"selected\">--请选择机构--</option>").appendTo(select);
			...
			<div id="Menu_Frame_Div" class="Menu_Frame_Div">
				<div style="float:left;width:100%;">
					<s:iterator value="serviceResult.operationList" id="sr">
						...
						<s:elseif test="%{#sr.operationType=='Upload'}">
							<select id="instList" name="id" style="float:left; margin-left:2px; font-family:'Microsoft Yahei'; font-size:11px;width:150px;">
							</select>
							...
							<a class="Menu_Frame_Div_Cell" href="javascript:ReportImportByAjax('<s:property value="operationNamespace"/>','<s:property value="operationAction"/>')">
								<s:property value="operationName" />
							</a>
						</s:elseif>
					</s:iterator>
