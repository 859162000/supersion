	...
	<head>
		...
		<script type="text/javascript">
			...
			var address = window.location.pathname;
	      	...
	      	//任务管理
	      	function _jmxTask(id,method) {
      			$.ajax({
      				type:"POST",
      				url: "jmx/jmxTask.action", 
...
<%--					WhenUserClikeDiv("SelectDivFrame");		lanyuesheng	20160203	据条件是否存在值显示/隐藏 --%>
	...
	<body onResize="AutiSize();" style="margin:0px; width: 100%;height:100%;" onload="ShowHeadName();" ondragstart="return false;">
		...
		<s:form name="form1" enctype="multipart/form-data" method="POST" id="from1">
			...
			<s:if test="serviceResult.showCondition.size()>0">
				<div id="SelectDivFrame" style="width: 100%; height:auto;border-bottom:1px solid #0060A6;font-size: 12px; font-family: 微软雅黑;clear: both; top:30px; background-color:White; z-index:1000;display: none;">
			...
			<div id="Table_Frame_Div" class="Table_Frame_Div">
				<table id="Grid_Task" class="Grid_Table" border="1px"  cellpadding="0" cellspacing="0" Rules=rows>
					<tr id="JS_Grid_Head_Row" class="JS_Grid_Head_Row">
						...
						<td title="监控" class="JS_Grid_Head_Row_Cell" style="width: 80px;">
							<div class="TD_Div_Nowrap">监控</div>
						</td>
					</tr>
					...
					<s:else>
						<s:iterator value="serviceResult.valueTable" id="vt" status="st">
							<tr class="JS_Grid_Data_Row" style="font-size: 12px; font-family: 微软雅黑;" Rows_Type='DataRow'>
								...
								<td title="监控" class="JS_Grid_Data_Row_CellLink" style="width: 80px;" columncode="ColA"> 
								     <a id="from1_" href="javascript:_jmxTask('<s:property value="%{#vt.get(0).value}"/>','start')" style="color:#4169E1;">采集</a>
								     <a id="from1_" href="javascript:_jmxTask('<s:property value="%{#vt.get(0).value}"/>','stop')" style="color:#4169E1;">停止</a>
								</td> 
							</tr>
						</s:iterator>
					</s:else>
