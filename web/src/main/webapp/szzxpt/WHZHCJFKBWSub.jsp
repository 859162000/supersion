...
<html>
	<head>
	    ...
	    /*function getColumnTotalWidth(){
	        ...
	        var Grid_TaskWindth = $(window).width() - subValue;
	        if(totalwidth < Grid_TaskWindth){
	            totalwidth = Grid_TaskWindth;
	        }
	        return totalwidth;
	    } */
			...
			new window.parent.ObjectWindows("Main_Body", 'qyzx' + "Windows", 520, 1000, url, '<s:property value="name" />', "no");
	        }
	...
	<body onResize="AutiSize();" style="margin:0px; width: 100%;height:100%;" onload="ShowHeadName();">
			...
			<div id="Menu_Frame_Div" class="Menu_Frame_Div">
				<div style="float:left;width:100%;">
					...
					<s:if test="serviceResult.showCondition.size()>0">
						<div class="Menu_Frame_Div_Cell" onclick ="WhenUserClikeDiv('SelectDivFrame');">高级查询</div>
					</s:if>
				</div>
								...
								<s:iterator value="#vt" status="fl">
									<s:if test="#fl.index!=0 && #fl.index!=1">
										...
										style="width: <s:property value="(width==null || width=='')?80:width"/>px;"
											...
											<s:if test="value.indexOf('openLinkedUrl')>=0">
												<a href="javascript:onOpenLinkClick('<%=basePath%><s:property value="value.substring(value.indexOf('修改数据')+5)"/>')">
													<s:property value="value.substring(value.indexOf('_')+1,value.indexOf('_')+5)" /></a>
											</s:if>
											<s:elseif test="value.indexOf('linkedUrl')>=0">
											    <a href="javascript:onLinkClick('<%=basePath%><s:property value="value.substring(value.indexOf('修改数据')+1)"/>')">
											<s:property value="value.substring(value.indexOf('修改数据')+1)" /></a>
											</s:elseif>
											<s:else>
