	...
	<head>
		...
		<script type="text/javascript">
		    ...
		    $(document).ready(function() {
		        ...
		        $("#from1_serviceParam_startDate").attr("title","FC_STAYEAR");
		        $("#from1_serviceParam_endDate").attr("title","FC_NOWDATE");
		        clearData();
				$("[id='serviceParam.dimensionType1']").change(function(){
					if($(this).val()=="01"){
						$("[id='serviceParam.timeFunc']").attr("disabled",false);
			        	$("#from1_serviceParam_startDate").attr("readonly",false);
					}else
						clearData();
				});
				$("[id='serviceParam.dimensionType2']").change(function(){
					if($(this).val()=="01"){
						$("[id='serviceParam.timeFunc']").attr("disabled",false);
			        	$("#from1_serviceParam_startDate").attr("readonly",false);
					}else
				...
				$("[id='serviceParam.timeFunc']").change(function(){
					$("#from1_serviceParam_endDate").attr("readonly",false);
					if($(this).val()!=""){
						if($(this).val()=="yearToNow"){
							$("#from1_serviceParam_startDate").attr("readonly",true).val("FC_STAYEAR");
							$("#from1_serviceParam_endDate").attr("readonly",true).val("FC_NOWDATE");
						}
						else{
							$("#from1_serviceParam_startDate").attr("readonly",true).val("");
						}
					}
					else{
						$("#from1_serviceParam_startDate").attr("readonly",false);
		    ...
		    function clearData(){
		    	var dtype1 = $("[id='serviceParam.dimensionType1']").val();
		        var dtype2 = $("[id='serviceParam.dimensionType2']").val();
		        if(dtype1 !="01" && dtype2 !="01"){
		        	$("[id='serviceParam.timeFunc']").attr("disabled",true).val("");
		        	$("#from1_serviceParam_startDate").attr("readonly",true).val("");
		        }
		    }
