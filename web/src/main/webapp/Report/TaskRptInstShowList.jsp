...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function onOperationInputClick(actionNamespace,actionName){
				 var reason="";
				 /*while(true)
				 {
				 	 reason=prompt("请输入强制提交原因","");
				 	  if(reason=="")
    			 		{
    			 			alert("原因必填");
				...
				// var url="<%=basePath%>" + actionNamespace + "/" + actionName + ".action";
				 //f.attr("action",url);
				 f.append("<input name='reason' type='hidden' value='"+reason+"'>")
				 //f.submit();
	        ...
	        $(document).ready(function(){
				...
				if($('#queryBtn').text() == '' || $('#queryBtn').text() == 'undefined'){
					...
					WhenUserClikeDiv("SelectDivFrame");
					$("#SelectDivFrame").hide();
					return false;
				}
	    });
    ...
    <script type="text/javascript">
		...
		/**
		 * description: 获取当前操作记录展示在界面上的备注信息
		 * updateDate: 2015-11-03 AM
		 */
		function getRemark(linkId, rowId){
			// 获取连接的名称
			var linkContent = $('#'+linkId).html();
		    // 获取table中的表头的所有td
			var theadTds = $("#Grid_Task tr:eq(0) td").toArray();
			// 定义索引对象，用于记录备注字段显示列的位置
			var index = null;
			if(theadTds.length > 0){
				for(var i = 2; i < theadTds.length; i++){
					var htmlVal = theadTds[i].innerHTML;
					var strTemp1 = htmlVal.split(">");
					var strTemp2 = strTemp1[strTemp1.length-2].split("<")[0];
					linkContent = $.trim(linkContent);
					strTemp2 = $.trim(strTemp2);
					if(linkContent.indexOf(strTemp2)>1){
						index = i;
						break;
					}
				}
			...
			if(index != null){
				// 获取当前操作行的所有TD节点
				var operateRow = $("#"+rowId);
				// 获取用于显示备注字段的TD
				var remarkTd = operateRow.children().toArray()[index];
				// 获取备注内容
				var remarkContent = remarkTd.innerHTML;
				
				// 分割备注内容
				var c1 = remarkContent.split("&nbsp;");
				// 返回备注信息
				return c1[1];
			}else{
				return null;			
		...
		// 设置备注字段的值
		function setRemark(linkId, rowId, value){
			// 获取连接的名称
			var linkContent = $('#'+linkId).html();
		    // 获取table中的表头的所有td
			var theadTds = $("#Grid_Task tr:eq(0) td").toArray();
			...
			if(index != null){
				...
				// 分割备注内容
				var c1 = remarkContent.split("&nbsp;");
				var remark = c1[0]+"&nbsp;"+value+"&nbsp;"+c1[2];
				remarkTd.innerHTML = remark;
				$(remarkTd).attr("title",value);
			}
		}
		...
		function closeDialogWin(){
			$('#popBox').hide();
			$('#dialog-overlay').hide(); 
			$('#remark').val("");//parent().html('<textarea id="remark" style="width:150px;height:100px;"></textarea>');
		}
		function dialogResize()
		{
				var windowHeight = $(window).height();
  			var windowWidth = $(window).width();
  			var win_top = (windowHeight - $('#popBox').height())/2;
  			var win_left = (windowWidth - $('#popBox').width())/2;
  			
  			$('#popBox').css({top:win_top, left:win_left}).show();
			$('#dialog-overlay').css({height:windowHeight, width:windowWidth}).show();
		}
		// 修改备注信息
  		function updateReportRemark(actionNameSpace, actionName, dataId, linkId, rowId){
  			$('#remark').val(getRemark(linkId, rowId));
  		    dialogResize();
  			
  			$('#saveBtn').unbind('click').click(function(){
  				var remarkValue = null;
  				remarkValue = $('#remark').val();
  				if(remarkValue == null || remarkValue == ''){
  					alert("请输入备注信息");
  					//$('#remark').parent().html('<textarea id="remark" style="width:150px;height:100px;"></textarea>');
  					return;
  				}else{
  					$('#remark').val("");//parent().html('<textarea id="remark" style="width:150px;height:100px;"></textarea>');
  					$('#popBox').hide();
					$('#dialog-overlay').hide(); 
		  			...
		  			strArr = actionName.split("-");
					$.ajax({
						url:url,    //请求的url地址
						dataType:"text/xml",   //返回格式为json
						async:true,//请求是否异步，默认为异步，这也是ajax重要特性
						data:{
							"dataId":dataId,
							"fieldName":'strRemark',
							"fieldvalue":remarkValue,
							"updateObjectName":strArr[strArr.length-1]
						},    //参数值
						type:"POST",   //请求方式
						complete:function(data){
							...
							alert(result[0]);
					  		if(result[1] == 'true'){
						    	setRemark(linkId, rowId, result[2]);
					  		}
						}
					});
  				}
  			...
  			$('#cansoleBtn').unbind('click').click(function(){
  				$('#popBox').hide();
				$('#dialog-overlay').hide(); 	
  			});
	...
	<style type="text/css">
		...
		#dialog-overlay {width:100%;height:100%;position:absolute;top: 0;left:-25px; background:#000; opacity:.20; filter:Alpha(Opacity=20);top:0; left:0; z-index:8000;display:none;}
	</style>
	</head>
	<body onResize="AutiSize();" style="margin:0px; width: 100%;height:100%;" onload="ShowHeadName();" ondragstart="return false;">
		...
		<div id="popBox">
			<table border="0" cellspacing="0" cellpadding="0">
				<!-- 第一行；用于显示窗体的名称（也可以说是窗体的提示信息） -->
				<tr>
					<td id="windowTips" class="win_tl"></td>
					<td class="win_tm"></td>
					<td class="win_tr"></td>
				</tr>
				<!-- 第二行，用于显示窗体中的数据 -->
				<tr>
					<td class="win_ml"></td>
					<td class="win_mm">
						<table border="0" cellspacing="0" cellpadding="0" style="width:300px; height:182px; background:white;">
							<tr style="height:20px;margin-top:3px;">
								<td style="text-align:left;padding-top:2px;font-size:12px;text-indent:1em; " colspan="3">请输入备注信息</td>
							</tr>
							<tr >
								<td colspan="3" >
									<textarea id="remark" style="width:290px;height:100px;margin-left:5px;margin-right:2px;"></textarea>
								</td>
							</tr>
							<tr style="height:30px;align:right;font-size:12px;">
							    <td style="width:50%"/>
								<td style="width:25%">
								<div id="saveBtn" style="cursor:pointer;width:50px;float:right;">确定</div>
								</td>
								<td style="width:25%">
								<div id="cansoleBtn" style="cursor:pointer;width:50px;float:left;" onclick="closeDialogWin();">取消</div>
								</td>
			...
			<div id="Menu_Frame_Div" class="Menu_Frame_Div">
				<div style="float:left;width:100%;">
					<s:iterator value="serviceResult.operationList" id="sr">
						...
						<s:if test="%{#sr.operationType=='PostInput'}">
							<a class="Menu_Frame_Div_Cell" href="javascript:onOperationInputClick('<s:property value="operationNamespace"/>','<s:property value="operationAction"/>')">
								<s:property value="operationName" /> 
							</a>
						</s:if>
					</s:iterator>
				...
				<table id="Grid_Task" class="Grid_Table" border="1px"  cellpadding="0" cellspacing="0" Rules=rows>
					<tr id="JS_Grid_Head_Row" class="JS_Grid_Head_Row">
						...
						<s:iterator value="serviceResult.linkedList.get(0)" id="linkedList" status="linkedListStatus">
							<td title="<s:property value="operationName" />" class="JS_Grid_Head_Row_Cell" style="width: <s:property value="(width==null || width=='')?(operationName.equals('导入')?250:100):width" />px;" columncode="Col1">
					...
					<s:else>
						<s:iterator value="serviceResult.valueTable" id="vt" status="st">
							<tr id="row_<s:property value="#st.index+1"/>" class="JS_Grid_Data_Row" style="font-size: 12px; font-family: 微软雅黑;" Rows_Type='DataRow'>
								...
								<s:iterator value="serviceResult.linkedList.get(#st.index)" id="ll">
									...
									<s:elseif test="%{#ll.operationType=='PostUpdate'}">
										<td title="<s:property value='operationName'/>" class="JS_Grid_Data_Row_CellLink" style="width: <s:property value="(width==null || width=='')?100:width" />px;" columncode="ColA">
										    <a id="link_<s:property value="#st.index+1"/>" href="javascript:updateReportRemark('<s:property value='%{#ll.operationNamespace}'/>','<s:property value='%{#ll.operationAction}'/>','<s:property value='%{#vt.get(0).value}'/>','link_<s:property value="#st.index+1"/>','row_<s:property value="#st.index+1"/>')"
	 											cssStyle="width: <s:property value='width==null?100:width' />px; color:%{#ll.color};">
												<s:if test="%{#ll.imageUrl==null||imageUrl.equals('')}">
													<s:property value="operationName" />
												</s:if>
